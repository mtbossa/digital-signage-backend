...
class PostRelationshipsTest extends TestCase
{
    ...
    public function check_if_post_belongs_to_many_displays_relationship_is_working()
    {
        ...
        foreach ($displays as $display) {
            $this->assertDatabaseHas('display_post',
                ['post_id' => $post->id, 'display_id' => $display->id]);
        }
    }
    ...
    public function ensure_can_attach_displays_when_creating_post()
    {
        ...
        $post_data = Post::factory()->nonRecurrent()
          ->make(['media_id' => $media->id])->toArray();

        $response = $this->postJson(route('posts.store'),
            [...$post_data, 'displays_ids' => $displays_ids->toArray()])
          ->assertCreated();

        foreach ($displays_ids as $key => $id) {
            $response->assertJson(['displays' => [$key => ['id' => $id]]]);
            $this->assertDatabaseHas('display_post',
                ['post_id' => $response['id'], 'display_id' => $id]);
        }
    }
    ...
    public function ensure_not_passed_displays_are_detached_when_updating_post()
    {
        ...
        $response = $this->putJson(route('posts.update', $post->id),
            [...$post->toArray(), 'displays_ids' => $displays_ids])
          ->assertOk();
        ...
        $this->assertDatabaseMissing('display_post',
            ['post_id' => $post->id, 'display_id' => $removed_display_id]);
    }
    ...
    public function ensure_all_displays_are_detached_when_updating_post_if_displays_ids_is_null()
    {
        ...
        $response = $this->putJson(route('posts.update', $post->id),
            [...$post->toArray(), 'displays_ids' => null])
          ->assertOk();

        foreach ($displays_ids as $display_id) {
            $this->assertDatabaseMissing('display_post',
                ['post_id' => $post->id, 'display_id' => $display_id]);
        }
    }
    ...
    {
        $media = Media::factory()->create();
        $post_data = Post::factory()->nonRecurrent()->make(['media_id' => $media->id, 'displays_ids' => null])->toArray();

        $this->postJson(route('posts.store'), $post_data)->assertCreated()->assertJson(['media_id' => $media->id]);
    ...
    public function should_link_recurrence_to_post_when_creating_post_with_recurrence()
    {
        ...
        $response = $this->postJson(route('posts.store'), $post_data)->assertCreated()
          ->assertJson(['recurrence_id' => $recurrence->id]);

        $this->assertDatabaseHas('posts',
            ['id' => $response['id'], 'recurrence_id' => $recurrence->id]);
    }
    ...
    public function a_post_might_belong_to_a_recurrence()
    {
        ...
        $this->assertDatabaseHas('posts',
            ['id' => $post->id, 'recurrence_id' => $recurrence->id]);
    }
    ...
    public function post_should_be_deleted_when_recurrence_is_deleted()
    {
        $media = Media::factory()->create();
        $recurrence = Recurrence::factory()->create();
        $non_recurrent_post = Post::factory()->nonRecurrent()
          ->create(['media_id' => $media->id]);
        $post = Post::factory()->create([
            'media_id' => $media->id,
            'recurrence_id' => $recurrence->id,
