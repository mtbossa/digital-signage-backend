...
class StoreTest extends TestCase
{
    ...
    public function create_store()
    {
        ...
        $this->postJson(route('stores.store'), $store_data)->assertCreated()
            ->assertJson($store_data);
    ...
    public function update_store()
    {
        ...
        $response = $this->putJson(route('stores.update', $this->store->id),
            $update_values);
    ...
    {
        $response = $this->deleteJson(route('stores.destroy',
            $this->store->id));
    ...
    {
        $correctData = $this->store->toArray();
        unset($correctData['plainTextToken']);
        $this->getJson(route('stores.show',
            $this->store->id))->assertOk()->assertJson($correctData);
    }
    ...
    public function fetch_all_stores()
    {
        ...
        $this->getJson(route('stores.index'))->assertOk()->assertJsonCount(2)
