...
class RaspberryAuthTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->raspberry
            = $this->_createRaspberry(['display_id' => $this->display->id]);
    }
    ...
    )
    {
        $response = $this->getJson(route('raspberry.posts.index',
            ...
            ['Authorization' => "Bearer {$this->raspberry->plainTextToken}"])
            ->assertOk();
    }
    ...
    {
        $this->getJson(route('raspberry.posts.index',
            ['raspberry' => $this->raspberry->id]))->assertUnauthorized();
    }
}
